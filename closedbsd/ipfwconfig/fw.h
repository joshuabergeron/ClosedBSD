...
enum { ADD, FLUSH, RAW }; // cmd
enum { ALLOW, DENY, DIVERT, FWD, RESET }; // action
enum { IP, TCP, UDP, ICMP }; // proto
enum { ANY, INCOMING, OUTGOING }; // ifs
enum { NONE, SETUP, ESTABLISHED, FRAG };        // tcp options
enum { IP_NONE, SSRR, LSRR, TS }; // ip options  
enum { ICMP_NONE, ECHO_REPLY, DEST_UNREACH, ECHO_REQUEST }; // icmp options
...
int forw_input(WINDOW *w, int selected, char src[128], char dst[128], char forw[128], char src_port
[5], char dst_port[5], char forw_port[5]);
